...
def main(file_path, feature_type, exercise, label_map, save_dir):
    ...
    patients_to_remove = [
        'HUMV_HC_1','HUMV_HC_3','HUMV_HC_4','HUMV_HC_5','HUMV_HC_6','HUMV_HC_7','HUMV_HC_8',
        'HUMV_HC_9','HUMV_HC_11','HUMV_HC_13','HUMV_HC_14','HUMV_HC_15','HUMV_HC_16','HUMV_HC_17',
        'HUMV_HC_20','HUMV_HC_24','HUMV_HC_25','HUMV_HC_26','HUMV_NFC_1','HUMV_NFC_3','HUMV_NFC_4',
        'HUMV_NFC_6','HUMV_NFC_10','HUMV_NFC_11','HUMV_NFC_12','HUMV_NFC_13','HUMV_NFC_15','HUMV_NFC_17',
        'HUMV_NFC_23','HUMV_NFC_24','HUMV_AC_2','HUMV_AC_5','HUMV_AC_7','HUMV_AC_8','HUMV_AC_13',
        'HUMV_AC_14','HUMV_AC_15','HUMV_AC_16','HUMV_AC_26','HUMV_AC_27','HUMV_AC_28','HUMV_AC_29',
        'HUMV_AC_31','HUMV_AC_1','HUMV_AC_3','HUMV_AC_25','HUMV_AC_33'
    ]

    # Remove patients that start with "HUMV_PD" OR are in the explicit list
    df = df[~df['id'].str.startswith("HUMV_PD")]
    df = df[~df['id'].isin(patients_to_remove)]
    ...
    X = df.drop(columns=['id', 'label', 'exercise', 'age', 'sex',  'segment']) #id,label,exercise,age,sex,segment
    y = df['label']
    groups = df['id']
    ...
    param_grids = {
        ...
        'poly': {'kernel': ['poly'], 'degree': [2, 3, 4, 5], 'gamma': [1e-5, 1e-4, 1e-3, 0.01, 0.1, 1, 10, 100]}
    }
    ...
    for kernel_name, param_grid in param_grids.items():
        ...
        for fold, (train_idx, test_idx) in enumerate(outer_cv.split(unique_patients, patient_labels, groups= unique_patients), 1): #
            ...
            # Count AC & HC in the training set
            train_AC_count = sum(patient_labels[train_idx] == 1)
            ...
            for inner_train_idx, inner_val_idx in inner_cv.split(unique_train_patients, train_patient_labels, groups=unique_train_patients):
                ...
                print(f"  Inner CV split: Train AC={inner_train_labels.count(1)}, Train HC={inner_train_labels.count(0)} | "
                    f"Val AC={inner_val_labels.count(1)}, Val HC={inner_val_labels.count(0)}")
            ...
            print(f"Fold {fold} best params: {best_params}, Final Train AC={train_AC_count}, Train HC={train_HC_count} ")
