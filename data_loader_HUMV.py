...
def load_audio_data(root_directory, start_with=None, exact_name = None, patient_type_mapping=mapping_dict, audio_extensions=['.wav']):
	...
	"""
	Loads audio file paths and their corresponding labels from a hierarchical directory structure 
	organized by patient types, allowing optional filtering of audio files based on name prefixes 
	or exact matches.

	Args:
	- root_directory (str): The root directory containing patient type folders (e.g., 'HC', 'NFC', etc.).
	- start_with (str, optional): If provided, only include audio files whose names start with this 
	  string (case-insensitive). Defaults to None.
	- exact_name (str, optional): If provided, only include the audio file that exactly matches this 
